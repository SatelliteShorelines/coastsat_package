...
# min_roi_coverage : any images that cover less than this percentage of the ROI will be ignored
SDS_download.check_images_available(inputs, min_roi_coverage=0.5)
...
# min_roi_coverage : any images that cover less than this percentage of the ROI will be ignored
metadata = SDS_download.retrieve_images(inputs, min_roi_coverage=0.5)
...
def load_data_from_json(filepath: str) -> dict:
    ...
    def DecodeDateTime(readDict):
        ...
        if "dates" in readDict:
            tmp = [datetime.fromisoformat(dates) for dates in readDict["dates"]]
            readDict["dates"] = tmp
...
reference_elevation = (
    0.7  # elevation at which you would like the shoreline time-series to be
)
beach_slope = 0.1
...
filepath = os.path.join(
    os.getcwd(), "examples", "NARRA_time_series_tidally_corrected.csv"
)
...
for i, key in enumerate(topo_profiles.keys()):
    ...
    ax.grid(linestyle=":", color="0.5")
    ax.plot(
        topo_profiles[key]["dates"],
        topo_profiles[key]["chainages"],
        "-o",
        ms=4,
        mfc="w",
    )
...
for key in cross_distance.keys():
    ...
    # save plot
    fig.savefig(
        os.path.join(os.getcwd(), "examples", "comparison_transect_%s.jpg" % key),
        dpi=150,
    )
...
for k, sat in enumerate(list(np.unique(satnames_all))):
    ...
    n_data.append(len(chain_error[idx]))
bp = ax[1].boxplot(
    data, 0, "k.", labels=list(np.unique(satnames_all)), patch_artist=True
)
for median in bp["medians"]:
    median.set(color="k", linewidth=1.5)
for j, boxes in enumerate(bp["boxes"]):
...
fig.savefig(
    os.path.join(os.getcwd(), "examples", "comparison_all_transects.jpg"), dpi=150
)
