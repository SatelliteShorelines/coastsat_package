...
def create_cloud_mask(im_QA, satname, cloud_mask_issue, collection):
    ...
    if sum(sum(cloud_mask)) > 0 and sum(sum(~cloud_mask)) > 0:
        morphology.remove_small_objects(cloud_mask, min_size=40, connectivity=1, out=cloud_mask)
        ...
        if cloud_mask_issue:
            ...
            morphology.remove_small_objects(cloud_mask, min_size=100, connectivity=1, out=cloud_mask)
