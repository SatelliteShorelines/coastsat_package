...
def get_filepath(inputs, satname):
    ...
    return [
        os.path.join(filepath_data, sitename, satname, path)
        for path in sat_paths[satname]
    ]
...
def get_filenames(filename, filepath, satname):
    ...
    """
    # Check if satellite name is recognized
    satname_to_replacement = ["L5", "L7", "L8", "L9", "S2"]
    if satname not in satname_to_replacement:
        raise ValueError(f"Unrecognized satellite name: {satname}")
