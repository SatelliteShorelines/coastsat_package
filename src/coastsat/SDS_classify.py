...
class SelectFromImage(object):
    ...
    def __init__(self, ax, implot, color=[1, 1, 1]):
        ...
        xv, yv = np.meshgrid(
            np.arange(self.array.shape[1]), np.arange(self.array.shape[0])
        )
...
def label_images(metadata, settings):
    ...
    # initialize figure
    fig, ax = plt.subplots(
        1, 1, figsize=[17, 10], tight_layout=True, sharex=True, sharey=True
    )
        ...
        for i in range(len(filenames)):
            ...
            # otherwise label this image
                ...
                while 1:
                    ...
                    if (
                        seed[0] > 0.95 * im_ms.shape[1]
                        and seed[1] < 0.05 * im_ms.shape[0]
                    ):
                    ...
                    # otherwise label the selected sand pixels
                        ...
                        fill_NDVI = flood(
                            im_NDVI, (seed[1], seed[0]), tolerance=settings["tolerance"]
                        )
                        fill_NDWI = flood(
                            im_NDWI, (seed[1], seed[0]), tolerance=settings["tolerance"]
                        )
                ...
                while 1:
                    ...
                    if (
                        ...
                        if len(ww_pixels) > 0:
                            ...
                            for k in range(im_viz.shape[2]):
                                im_viz[ww_pixels[-1][1], ww_pixels[-1][0], k] = im_RGB[
                                    ww_pixels[-1][1], ww_pixels[-1][0], k
                                ]
                            implot.set_data(im_viz)
                            fig.canvas.draw_idle()
                            del ww_pixels[-1]
                ...
                while True:
                    ...
                    if key_event.get("pressed") == "enter":
                        ...
                        selector_land.im_bool = np.zeros(
                            (selector_land.array.shape[0], selector_land.array.shape[1])
                        )
                        selector_land.ind = []
                # update im_viz and im_labels
                im_viz = selector_land.array
                selector_land.im_bool = selector_land.im_bool.astype(bool)
                im_labels[selector_land.im_bool] = settings["labels"][
                    "other land features"
                ]
        ...
        for file in list_files_pkl:
            ...
            for key in labelled_data["features"].keys():
                if len(labelled_data["features"][key]) > 0:  # check that is not empty
                    # append rows
                    features[key] = np.append(
                        features[key], labelled_data["features"][key], axis=0
                    )
...
def evaluate_classifier(classifier, metadata, settings):
    ...
    plt.ioff()
    fig, ax = plt.subplots(
        1, 2, figsize=[17, 10], sharex=True, sharey=True, constrained_layout=True
    )
            ...
            try:  # use try/except structure for long runs
                if sum(sum(im_labels[:, :, 0])) < 10:
                    ...
                    contours_mwi, t_mndwi = SDS_shoreline.find_wl_contours2(
                        im_ms, im_labels, cloud_mask, buffer_size_pixels, im_ref_buffer
                    )
            except:
